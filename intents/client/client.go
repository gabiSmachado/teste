...
func (client *Client) IntentCreate(intent datamodel.Intent) (int, error) {
	...
	if err != nil {
		fmt.Errorf("failed to marshal intent (%s)", err)
		return -1, err
	}
	...
	if err != nil {
		fmt.Errorf("failed to send intent (%s)", err)
		return -1, err
	}
	...
	case 200:
		...
		if err != nil {
			fmt.Errorf("failed to parse response (%s)", err)
			return -1, err
		}
...
func (client *Client) IntentDelete(idx int) error {
	if idx < 0 {
		fmt.Errorf("invalid intent %d", idx)
	...
	if err != nil {
		fmt.Errorf("failed to create delete request %s", err)
		return err
	}

	res, err := SendCommand(req)
	if err != nil {
		fmt.Errorf("failed to send delete command %s", err)
		return err
	}
...
func (client *Client) IntentShow(idx int) (*datamodel.Intent, error) {
	...
	if err != nil {
		fmt.Errorf("failed to get intent %s", err)
		return nil, err
	}
